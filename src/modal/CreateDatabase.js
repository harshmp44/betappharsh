...
const CreateWingo = async(req, res) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        await connection.execute(sql, ['2024060110000', arr[i], timeNow]);
        const sql_1 = "INSERT INTO wingo SET period = ?, game = ?, amount = 0, status = 0, time = ?";
        await connection.execute(sql_1, ['2024060110001', arr[i], timeNow]);
    }
...
const Create5D = async(req, res) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        await connection.execute(sql, ['2024060110000', '23521', arr[i], timeNow]);
        const sql_1 = "INSERT INTO 5d SET period = ?, result = ?, game = ?, status = 0, time = ?";
        await connection.execute(sql_1, ['2024060110001', '0', arr[i], timeNow]);
    }
...
const CreateK3 = async(req, res) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        await connection.execute(sql, ['2024060110000', '235', arr[i], timeNow]);
