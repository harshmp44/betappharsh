...
const addBankUSDTPage = async (req, res) => {
  return res.render("wallet/addbankUSDT.ejs");
};
...
const promotionPage = async (req, res) => {
  ...
  if (userRecord) {
    const [directInvitesRes] = await connection.query(
      "SELECT * FROM invites WHERE user_id = ? AND level = ?",
      [userRecord.id, 1]
    );

    directReferral = directInvitesRes?.length;

    const [teamInvitesRes] = await connection.query(
      "SELECT * FROM invites WHERE user_id =? AND level != ?",
      [userRecord.id, 1]
    );

    totalMembers = teamInvitesRes?.length + directReferral;

    directCommissionAmount = userRecord?.direct_commission_amount;
    teamCommissionAmount = userRecord?.team_commission_amount;

    totalCommission = directCommissionAmount + teamCommissionAmount;
  }
