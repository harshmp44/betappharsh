...
const addFundsToReferralUserAsPerLevels = async ({
  userRecord,
  gameAmount,
}) => {
  let getInvitesQuery = `SELECT invites.*, level.id as level_id, level.f1 as level_percents FROM invites INNER JOIN level ON invites.level_id = level.id WHERE new_user_id =?`;

  const [invitesRes] = await connection.query(getInvitesQuery, [
    userRecord?.id,
  ]);

  const addMoneyToUsersAccount = async ({ user_id, percents, level }) => {
    let depositAmount = Number(gameAmount);
    ...
    const _userRecord = userRes?.[0];

    if (!_userRecord) {
      return true;
    }

    let money = Number(_userRecord?.money);
    let totalMoney = Number(_userRecord?.total_money);
    let updatedPercents = Number(percents);
    let commissionAmount = (depositAmount * updatedPercents) / 100;

    let directCommissionAmount = Number(_userRecord?.direct_commission_amount);
    let teamCommissionAmount = Number(_userRecord?.team_commission_amount);

    if (level == 1) {
      directCommissionAmount = directCommissionAmount + commissionAmount;
    } else {
      teamCommissionAmount = teamCommissionAmount + commissionAmount;
    }

    money = Math.round(money + commissionAmount);
    totalMoney = Math.round(totalMoney + commissionAmount);

    await connection.query(
      "UPDATE users SET money = ?, total_money = ?, direct_commission_amount = ?, team_commission_amount = ? WHERE id = ? ",
      [money, totalMoney, directCommissionAmount, teamCommissionAmount, user_id]
    );

    return true;
  };

  let addMoneyPromises = [];

  invitesRes.forEach((invite) => {
    addMoneyPromises.push(() =>
      addMoneyToUsersAccount({
        user_id: invite.user_id,
        percents: invite.level_percents,
        level: invite?.level,
      })
    );
  });

  await Promise.all(addMoneyPromises.map((_func) => _func()));
...
const betK5D = async (req, res) => {
  try {
    ...
    const [user] = await connection.query(
      "SELECT `phone`, `code`, `invite`, `level`, `money`, `id` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ",
      [auth]
    );
    ...
    if (check >= 0) {
      ...
      await addFundsToReferralUserAsPerLevels({
        gameAmount: money,
        userRecord: userInfo,
      });
