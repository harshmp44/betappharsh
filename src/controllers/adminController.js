...
import md5 from "md5";
import EnvEnums from "../enums/env.enums";
...
const filteredMember = async (req, res) => {
  
  let phone = req.body.phone;
  
  const query=`SELECT * FROM users WHERE veri = 1 AND level != 2 and phone like '%${phone}%' ORDER BY time DESC`;
...
const listMember = async (req, res) => {
  ...
  const [users] = await connection.query(
    `SELECT * FROM users WHERE veri = 1 AND level != 2 ORDER BY time DESC LIMIT ${pageno}, ${limit} `
  );
...
const listCTV = async (req, res) => {
  ...
  const [wingo] = await connection.query(
    `SELECT * FROM users WHERE veri = 1 AND level = 2 ORDER BY time DESC LIMIT ${pageno}, ${pageto} `
  );
  return res.status(200).json({
    message: "Success",
    status: true,
...
const rechargeDuyet = async (req, res) => {
  ...
  const [depositRecordRes] = await connection.query(
    `SELECT * FROM recharge WHERE id = ?`,
    [id]
  );

  let depositRecord = depositRecordRes?.[0];
  ...
  let userPhone = depositRecord?.phone;

  const [allDepositsForUser] = await connection.query(
  ...
  if (type == "confirm") {
    ...
    if (allDepositsForUser.length == 1) {
      const [userRes] = await connection.query(
        `SELECT * from users WHERE phone = ?`,
        [userPhone?.trim()]
      );

      const userRecord = userRes?.[0];
      ...
      // Process X% of commission for referral user on first time deposit from new user

      const commissionPercents =
        EnvEnums.firstTimeDepositCommissionToReferralUser;

      let commissionAmount = Math.round(
        (info[0].money * commissionPercents) / 100
      );

      await connection.query(
        `UPDATE users SET money = money + ?, total_money = total_money + ? WHERE code = ? `,
        [commissionAmount, commissionAmount, userRecord?.invite]
      );
    }
...
const getTotalEarning = async(req, res) =>{
  const [minute_sum] = await connection.query(
    `select sum(fee) as minute_sum from minutes_1`
  );
  const [result5d_sum] = await connection.query(
    `select sum(fee) as result5d_sum from result_5d`
  );
  const [result_k3_sum] = await connection.query(
    `select sum(fee) as resultk3_sum from result_k3`
  );
  const [withdraw_tax] = await connection.query(
    `SELECT sum(tax) as withdraw_tax FROM withdraw`
  );
  const [transfer_tax] = await connection.query(
    `SELECT sum(tax) as transfer_tax FROM balance_transfer`
  );

  let totalSum = minute_sum[0].minute_sum + result5d_sum[0].result5d_sum + result_k3_sum[0].resultk3_sum
                  + parseFloat(withdraw_tax[0].withdraw_tax) + transfer_tax[0].transfer_tax

  console.log('rows', totalSum);
  res.status(200).json({
    sum: totalSum.toFixed(2),
    minute_sum, result5d_sum,result_k3_sum,withdraw_tax, transfer_tax
  })
}
