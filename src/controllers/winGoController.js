...
const addFundsToReferralUserAsPerLevels = async ({
  ...
  const addMoneyToUsersAccount = async ({ user_id, percents, level }) => {
    ...
    let money = Number(userRecord?.money);
    let totalMoney = Number(userRecord?.total_money);
    let updatedPercents = Number(percents);
    let commissionAmount = (depositAmount * updatedPercents) / 100;

    let directCommissionAmount = Number(userRecord?.direct_commission_amount);
    let teamCommissionAmount = Number(userRecord?.team_commission_amount);
...
const betWinGo = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`, `id`, `invite`, `level`, `money` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ",
    [auth]
  );
