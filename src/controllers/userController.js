...
const addBankUSDT = async (req, res) => {
    let auth = req.cookies.auth;
    let name_bank = 'USDT';
    let name_user = 'NA';
    let stk = req.body.stk;
...
const infoUserBank = async (req, res) => {
    ...
    let result = 0;
    console.log('total', total, 'total2' , total2)
    // if (total - total2 > 0) 
    result = total - total2 - fee;
...
const withdrawal3 = async (req, res) => {
    ...
    // const [withdraw] = await connection.query('SELECT * FROM withdraw WHERE `phone` = ? AND DATE(`today`) = DATE(?)', [userInfo.phone, checkTime]);
    
    //only for testing - commment after test
    const withdraw = [1];
            ...
            if (userInfo.money - money >= 0) {
                if (result == 0) {
                    if (total - total2 >= 0) {
                        ...
                        console.log('withdraw.length', withdraw.length)
                    ...
                    time = ?,
                    tax = ?`;
                        await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, infoBank.name_bank, infoBank.email, infoBank.name_user, 0, checkTime, dates, tax]);
...
const transfer = async (req, res) => {
    ...
    let sender_money = userInfo.money;
    if (!user) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
    ...
    if (result == 0) {
        if (sender_money >= amount) {
            ...
            if (receiver.length === 1 && sender_phone !== receiver_phone) {
                ...
                amount = amount * 0.98;
                const sql =
                 "INSERT INTO balance_transfer (sender_phone, receiver_phone, amount, tax) VALUES (?, ?, ?, ?)";
                await connection.execute(sql, [sender_phone, receiver_phone, amount, tax]);
